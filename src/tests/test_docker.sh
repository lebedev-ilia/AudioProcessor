...
run_tests() {
    ...
    # Test 4: Unified API test
    print_status "Running Unified API test..."
    if docker-compose -f $compose_file exec -T audio-processor python test_unified_processor.py; then
        print_success "Unified API test passed"
    else
        print_error "Unified API test failed"
        return 1
    fi
    
    # Test 5: Unified API demo
    print_status "Running Unified API demo..."
    if docker-compose -f $compose_file exec -T audio-processor python demo_unified_api.py; then
        print_success "Unified API demo passed"
    else
        print_error "Unified API demo failed"
        return 1
    fi
